...
async function loadIntoTablesBody(url, tableBody) {
    ...
    for (const user of users) {
        ...
        bodyElement.appendChild(createTd(user.position));
...
function editModalOpen(e, id) {
    ...
    fetch('http://localhost:8080/rest/api/users/' + id, {
        ...
        .then((data) => {
            ...
            let controlPosition = document.getElementById('positionEdit')
            ...
            fillValuesFromData(controlId, controlName, controlPosition, controlEmail, controlUsername, data)
            controlPassword.value = data.password
        })
}

function fillValuesFromData(controlId, controlName, controlPosition, controlEmail, controlUsername, data) {
    controlId.value = data.id
    controlName.value = data.name
    controlPosition.value = data.position
...
function deleteModalOpen(e, id) {
    ...
    fetch('http://localhost:8080/rest/api/users/' + id, {
        ...
        .then((data) => {
            ...
            let controlDelPosition = document.getElementById('positionDelete')
            ...
            reSelectRolesOption(optionSelect, data)
            fillValuesFromData(controlDelId, controlDelName, controlDelPosition, controlDelEmail, controlDelUsername, data)
        })
}
